...
class Model(object):
    ...
    def __init__(self):
        ...
        self.nsamples = 3 # number samples MC
    ...
    def train(self, dataset: torch.utils.data.Dataset):
        ...
        valid_loader = torch.utils.data.DataLoader(
            valid_set, batch_size=self.batch_size, shuffle=True, drop_last=True
        )
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    log_prior, log_var_posterior, log_likelihood = self.forward_pass_MCsamples(batch_x,batch_y)
                    

                    #print("batch_x")
                    #print(batch_x)
                    #log_likelihood = Normal(output_features, 1.0).log_prob(batch_x).sum()
                    #log_likelihood = F.nll_loss(F.log_softmax(output_features, dim=1), batch_y, reduction='sum')
                    #log_likelihood = nll_loss(output_features, batch_y)

                    loss = (1/num_batches) * (log_var_posterior - log_prior) + log_likelihood /self.batch_size
            ...
            # Early stopping 
            current_val_loss = self.validation(valid_loader)
            print(current_val_loss)
            if current_val_loss - the_last_val_loss > 5:
    ...
    def validation(self, valid_loader):
        ...
        for batch_idx, (batch_x, batch_y) in enumerate(valid_loader):
            with torch.no_grad():
                log_prior, log_var_posterior, log_likelihood = self.forward_pass_MCsamples(batch_x,batch_y)
                val_loss = (1/num_batches) * (log_var_posterior - log_prior) + log_likelihood /self.batch_size
        return val_loss
    
    def forward_pass_MCsamples(self, x, y):
        
        current_features = x
        s_log_prior = torch.tensor(0.0)
        s_log_variational_posterior = torch.tensor(0.0)
        s_log_likelihood = torch.tensor(0.0)

        for s in range(self.nsamples):
            output, sample_log_prior, sample_log_variational_posterior = self.network(x)
            sample_log_likelihood = F.nll_loss(F.log_softmax(output, dim=1), y, reduction='sum')
            #sample_log_likelihood = F.nll_loss(output, y, reduction='sum')
            s_log_prior += sample_log_prior
            s_log_variational_posterior += sample_log_variational_posterior
            s_log_likelihood += sample_log_likelihood

        return s_log_prior/self.nsamples, s_log_variational_posterior/self.nsamples, s_log_likelihood/self.nsamples
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        #self.epsilon_dist= UnivariateGaussian(torch.tensor(0.0), torch.tensor(1.0))
        ...
        #  )
        #self.weights_var_posterior = MultivariateDiagonalGaussian(nn.Parameter(torch.ones(self.out_features, self.in_features) * torch.Tensor(1,1).uniform_(-0.2, 0.2)), 
                                                               #nn.Parameter(torch.ones(self.out_features, self.in_features)* torch.Tensor(1,1).uniform_(-5, -4)))
        #self.weights_var_posterior = MultivariateDiagonalGaussian(nn.Parameter(torch.zeros(self.out_features, self.in_features)), 
                                                               #nn.Parameter(torch.ones(self.out_features, self.in_features)))

        self.weights_var_posterior = MultivariateDiagonalGaussian(nn.Parameter(torch.Tensor(self.out_features, self.in_features).uniform_(-0.2, 0.2)), 
                                                               nn.Parameter(torch.Tensor(self.out_features, self.in_features).uniform_(-5, -4)))

        #self.w_mu = nn.Parameter(torch.Tensor(self.out_features, self.in_features).uniform_(-0.2, 0.2))
        #self.w_rho = nn.Parameter(torch.Tensor(self.out_features, self.in_features).uniform_(-5, -4))

        #if self.use_bias:
         #   self.b_mu = nn.Parameter(torch.Tensor(self.out_features).uniform_(-0.2, 0.2))
          #  self.b_rho = nn.Parameter(torch.Tensor(self.out_features).uniform_(-5, -4))
        #else:
            #self.b_mu = torch.zeros(self.out_features)
            #self.b_rho = torch.zeros(self.out_features)
        ...
        if self.use_bias:
            ...
            #self.bias_var_posterior = MultivariateDiagonalGaussian(nn.Parameter(torch.ones(self.out_features)*torch.Tensor(1,1).uniform_(-0.2, 0.2)), 
                                                                    #nn.Parameter(torch.ones(self.out_features)*torch.Tensor(1,1).uniform_(-5, -4)))

            #self.bias_var_posterior = MultivariateDiagonalGaussian(nn.Parameter(torch.zeros(self.out_features)), 
                                                                    #nn.Parameter(torch.ones(self.out_features)))
            self.bias_var_posterior = MultivariateDiagonalGaussian(nn.Parameter(torch.Tensor(self.out_features).uniform_(-0.2, 0.2)), 
                                                                    nn.Parameter(torch.Tensor(self.out_features).uniform_(-5, -4)))                                        
            assert isinstance(self.bias_var_posterior, ParameterDistribution)
            assert any(True for _ in self.bias_var_posterior.parameters()), 'Bias posterior must have parameters'
        else:
            self.bias_var_posterior = torch.zeros(self.out_features)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        weights = self.weights_var_posterior.sample() # * self.epsilon_dist.sample()
        bias = self.bias_var_posterior.sample() # * self.epsilon_dist.sample()

        #weights_sigma = torch.log(1 + torch.exp(self.w_rho))
        #bias_sigma = torch.log(1 + torch.exp(self.b_rho))


        #weights = self.w_mu + torch.mul( weights_sigma , weights_epsilon)
        #bias = self.b_mu + torch.mul(bias_sigma , bias_epsilon)
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            ...
            new_features, log_prior_sub, log_variational_posterior_sub = current_layer.forward(current_features)
            log_prior += log_prior_sub
            log_variational_posterior += log_variational_posterior_sub
            #print(new_features.shape, current_features.shape)
        ...
        #log_prior = log_prior/ len(self.layers)
        #log_variational_posterior = log_variational_posterior / len(self.layers)
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, sigma: torch.Tensor):
        ...
        self.mu = mu
        self.sigma = sigma
        self.sigma2  = sigma / 40
        self.pi = 1

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        return torch.log(torch.exp(Normal(self.mu,self.sigma).log_prob(values)) * self.pi + torch.exp(Normal(self.mu,self.sigma2).log_prob(values))  * (1-self.pi))

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        #print(self.sigma[self.sigma < 0].size == 0)
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        #self.epsilon = Normal(torch.zeros(self.rho.shape), torch.ones(self.rho.shape)).sample()
    
    #def sigma(self):
        #print(torch.log1p(torch.exp(self.rho)).type())
        #return torch.log1p(torch.exp(self.rho))
        

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        return Independent(Normal(loc = self.mu ,scale = self.sigma),1).log_prob(values)

        #return -math.log(math.sqrt(2*math.pi)) - torch.log(self.sigma) -((values - self.mu)**2)/(2*self.sigma**2).sum()

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        #return MultivariateNormal(loc=self.mu, covariance_matrix=self.rho).sample()
        #print(torch.log(1+torch.exp(self.rho)))
        #print(self.rho[self.rho < 0].size == 0)
        #return Independent(Normal(loc = self.mu ,scale = softplus(self.rho)),1).sample()
        #print(self.normal.sample(self.rho.size()))
